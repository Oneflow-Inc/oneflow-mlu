...
def _test_sparse_softmax_cross_entropy(
    ...
    of_output_mlu.sum().backward()
    print(of_logits_mlu.grad)
    assert np.allclose(
        of_output_mlu.cpu().numpy(), of_output_cpu.numpy(), rtol=1e-03, atol=1e-04
    )


def _test_sparse_softmax_cross_entropy_grad(device_type, data_type, label_type, batch_size, num_classes):
    ...
    print('*'*100)
    y_grad = flow.tensor(
        np.random.randn(*of_output_mlu.shape), device=flow.device(device_type), dtype=data_type
    ).requires_grad_(True)
    mlu_dx = flow.autograd.grad(
        outputs=of_output_mlu,
        inputs=of_logits_mlu,
        grad_outputs=y_grad,
    )[0]
    
    cpu_dx = flow.autograd.grad(
        outputs=of_output_cpu,
        inputs=of_logits_cpu,
    ...
    print()
    assert np.allclose(
        mlu_dx.cpu().numpy(), cpu_dx.numpy(), rtol=1e-03, atol=1e-04
    )
...
class TestSparseSoftmaxCrossEntropyWithLogits(flow.unittest.TestCase):
    ...
    def test_sparse_softmax_cross_entropy(test_case):
        ...
        arg_dict["batch_size"] = [8, 16]
        arg_dict["num_classes"] = [2, 1000]
