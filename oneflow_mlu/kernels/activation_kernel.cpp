...
static void CnnlActivationBackwardWithX(user_op::KernelComputeContext* ctx,
                                        const user_op::Tensor* dy, const user_op::Tensor* x,
                                        user_op::Tensor* dx,
                                        const CnnlActivationDescriptor& activation_desc) {
  CnnlTensorDescriptor diff_y_desc(dy), x_desc(x), diff_x_desc(dx);
  OF_CNNL_CHECK(cnnlActivationBackward(ctx->stream()->As<ep::MluStream>()->cnnl_handle(),
                                       ...
                                       /*diff_y*/ dy->dptr(),
                                       /*x_desc*/ x_desc.desc(),
                                       /*x. when op=relu_grad, replace x with y*/ x->dptr(),
                                       /*beta*/ nullptr,
                                       /*diff_x_desc*/ diff_x_desc.desc(),
                                       /*diff_x*/ dx->mut_dptr()));
}

static void CnnlActivationBackwardWithY(user_op::KernelComputeContext* ctx,
                                        const user_op::Tensor* dy, const user_op::Tensor* y,
                                        user_op::Tensor* dx,
                                        const CnnlActivationDescriptor& activation_desc) {
...
REGISTER_USER_KERNEL("tanh_grad")
    .SetCreateFn([]() {
      return user_op::NewOpKernel<MluActivationKernel>([](user_op::KernelComputeContext* ctx) {
        ...
        CnnlActivationDescriptor activation_desc;
        activation_desc.set(CNNL_ACTIVATION_TANH, /*prefer=*/CNNL_ACTIVATION_HIGH_PRECISION,
                            /*nanProp=*/CNNL_NOT_PROPAGATE_NAN, /*ceof=*/0.0);
        CnnlActivationBackwardWithY(ctx, dy, y, dx, activation_desc);
      });
    })
    .SetIsMatchedHob(BaseActivationIsMatched("dx"));
