...
template<typename T>
class MluGeluKernel final : public user_op::OpKernel {
 public:
  MluGeluKernel() = default;
  ~MluGeluKernel() = default;
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    cnnlNanPropagation_t gelu_nan_opt = CNNL_NOT_PROPAGATE_NAN;
    float coef = 1.0;
    ...
    OF_CNNL_CHECK(cnnlSetActivationDescriptor(activation_desc, mode, gelu_nan_opt, coef));
...
#define REGISTER_GELU_MLU_KERNEL(dtype)                                             \
  REGISTER_USER_KERNEL("gelu").SetCreateFn<MluGeluKernel<dtype>>().SetIsMatchedHob( \
      (user_op::HobDeviceType() == DeviceType::kMLU)                                \
      && (user_op::HobDataType("in", 0) == GetDataType<dtype>::value));

REGISTER_GELU_MLU_KERNEL(float)
REGISTER_GELU_MLU_KERNEL(float16)
