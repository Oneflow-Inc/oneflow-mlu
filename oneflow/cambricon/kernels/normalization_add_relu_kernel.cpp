...
class MluNormalizationAddReluKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    const void *addend_ptr = nullptr;
    if (addend != nullptr){
      addend_ptr = addend->dptr();
    }
    ...
    if (addend != nullptr){
      addend_desc.set(ndim, shape.data(), cnnl_data_type, CNNL_LAYOUT_NHWC);
    }
    int64_t dims[1] = {shape[ndim - 1]};
    weight_bias_mean_var_desc.set(1, dims, ConvertToCnnlDataType(gamma->data_type()), CNNL_LAYOUT_ARRAY);
    ...
    OF_CNNL_CHECK(cnnlGetBatchNormForwardWorkspaceSize(stream->cnnl_handle(), x_desc.desc(), &bn_workspace_size));
    ...
    if constexpr (Type == BatchNormType::kTraining) {
      ...
      if (addend != nullptr){
        OF_CNNL_CHECK(cnnlBatchNormForwardTrainingV2(
          stream->cnnl_handle(), activation_desc.desc(), CNNL_BATCHNORM_SPATIAL, CNNL_BATCHNORM_OPS_BN_ADD_ACTIVATION, nullptr, nullptr, x_desc.desc(), x_ptr,
          addend_desc.desc(), addend_ptr, weight_bias_mean_var_desc.desc(), gamma->dptr(), beta->dptr(), moving_mean_ptr,
        ...
        OF_CNNL_CHECK(cnnlBatchNormForwardTrainingV2(
          stream->cnnl_handle(), activation_desc.desc(), CNNL_BATCHNORM_SPATIAL, CNNL_BATCHNORM_OPS_BN_ACTIVATION, nullptr, nullptr, x_desc.desc(), x_ptr,
          nullptr, nullptr, weight_bias_mean_var_desc.desc(), gamma->dptr(), beta->dptr(), moving_mean_ptr,
          moving_variance_ptr, epsilon, momentum, y_desc.desc(), y_ptr, mean->mut_dptr(),
          inv_variance->mut_dptr(), workspace.dptr(), bn_workspace_size, nullptr, 0));
      }
    } else {
      ...
      if (addend != nullptr){
        OF_CNNL_CHECK(cnnlBatchNormForwardInferenceV2(
          stream->cnnl_handle(), activation_desc.desc(), CNNL_BATCHNORM_SPATIAL, CNNL_BATCHNORM_OPS_BN_ADD_ACTIVATION, nullptr, nullptr, x_desc.desc(), x_ptr,
          weight_bias_mean_var_desc.desc(), gamma->dptr(), beta->dptr(), addend_desc.desc(), addend_ptr, moving_mean_ptr,
          moving_variance_ptr, epsilon, y_desc.desc(), y_ptr));
      }
      else{
        OF_CNNL_CHECK(cnnlBatchNormForwardInferenceV2(
          stream->cnnl_handle(), activation_desc.desc(), CNNL_BATCHNORM_SPATIAL, CNNL_BATCHNORM_OPS_BN_ACTIVATION, nullptr, nullptr, x_desc.desc(), x_ptr,
          weight_bias_mean_var_desc.desc(), gamma->dptr(), beta->dptr(), nullptr, nullptr, moving_mean_ptr,
          moving_variance_ptr, epsilon, y_desc.desc(), y_ptr));
      }
    }
    if (axis == 1) {
